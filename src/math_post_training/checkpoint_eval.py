...
def select_checkpoints(root, *, every=1, min_step=None, max_step=None):
    """Return an ordered, explicitly filtered sequence of Trainer checkpoints."""
    ...
    checkpoints = [
        ...
        if (min_step is None or checkpoint_step(checkpoint) >= min_step)
        and (max_step is None or checkpoint_step(checkpoint) <= max_step)
    ]
